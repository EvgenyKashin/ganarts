...
class Network:
    ...
    def run(self,
        ...
        key = dict(input_transform=input_transform, output_transform=output_transform, num_gpus=num_gpus, assume_frozen=assume_frozen, is_cpu=is_cpu, dynamic_kwargs=dynamic_kwargs)
        def unwind_key(obj):
        ...
        if key not in self._run_cache:
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.control_dependencies(None):
                ...
                for gpu in range(num_gpus):
                    device = tf.device("/cpu:0") if is_cpu else tf.device("/gpu:%d" % gpu)
                    with device:
